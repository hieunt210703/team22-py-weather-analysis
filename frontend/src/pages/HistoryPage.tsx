...
import { type LocationItem } from '../api/weatherApi';
...
export const HistoryPage: React.FC<HistoryPageProps> = ({ currentLocation }) => {
  ...
  if (!archive) {
    if (isError) return <ErrorMessage locationName={currentLocation.name} dataType="dữ liệu lịch sử" onRetry={() => { void refetch(); }} />;
    return (
